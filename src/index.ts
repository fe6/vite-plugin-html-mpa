    ...
    configureServer(server: ViteDevServer) {
      return () => {
        server.middlewares.use(async (req, res, next) => {
          const { pages: optPages } = options;
          const pageKeys = Object.keys(optPages);
          // if not html, next it.
          if (
            !pageKeys.find((item) => req.originalUrl?.endsWith(item)) &&
            req.originalUrl !== '/'
          ) {
            return next();
          }
          ...
          const pageName = (() => {
            ...
            return url?.substr(1) || 'index';
          })();
          const page = optPages[pageName] || {};
